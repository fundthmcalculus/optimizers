...
def plot_states(
    ...
    [ax[i].set_xlabel("time (s)") for i in [2, 3]]
    [
        ax[i].set_ylabel(r"$\dot{" + f"\\{var}" + "}$ (deg/s)")
...
def animate(y: np.ndarray, parameters: dict, path: str, **func_animate_kwargs) -> None:
    """Animates simulation.
    ...
    xlim = (
        1.5 * np.max(np.abs([r_s[0], r_t[0]]))
        + np.max([rho[0], rho[3]])
        + 2.0 * np.sum(rho[1:-1])
    )
    ylim = (
        1.5 * np.max(np.abs([r_s[1], r_t[1]]))
        + np.max([rho[0], rho[3]])
        + 2.0 * np.sum(rho[1:-1])
    )
