...
# ----------------------------------------------------------------------
def get_gam2_vals(
    y: np.ndarray, rho: np.ndarray, r_s: np.ndarray, r_t: np.ndarray
    ...
    for i, cp in enumerate(cps):
        ...
        djk_dot[i] = (
            np.sqrt(
                (v_cps[i][0] - v_ops[ind1][0]) ** 2
                + (v_cps[i][1] - v_ops[ind1][1]) ** 2
            ),
            np.sqrt(
                (v_cps[i][0] - v_ops[ind2][0]) ** 2
...
def calc_clik_params(
    ...
    """
    lam = calc_lambda(
        y, rho, r_s, p, q_bar, q_max, q_min, w, pos[:, 0, :], pos[:, 1, :]
    )
    lam_dot = calc_lambda_dot(
        y,
        rho,
        r_s,
        p,
