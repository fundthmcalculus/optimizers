...
class System:
    def __init__(
        self, y0: np.ndarray, parameters: dict, settings: dict = dict()
    ) -> None:
        ...
        self.t_dur = settings.get("t_dur", 15.0)
        self.h = settings.get("step_size", 0.01)
        self.tol = settings.get("tol", 0.01)
        self.max_tau = np.array(settings.get("max_tau", (10.0, 10.0, 10.0, 0.0)))
    ...
    def dynamics(self, t: float, y: np.ndarray, u: np.ndarray) -> np.ndarray:
        ...
        """
        yddot = forward_dynamics(
            y, u, self.r_s, self.r_t, self.rho, self.m, self.I, self.d
        ).squeeze()
    ...
    def run(self):
        ...
        sol = integrate(
            self.dynamics, (0.0, self.t_dur), self.y0, self.h, self.controls, event
        )
