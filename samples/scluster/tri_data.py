...
def main():
    ...
    if not os.path.exists('distance_matrix-uint8.npy'):
        ...
        if os.path.exists('distance_matrix.npy'):
            ...
            data = np.loadtxt(input_file, dtype=default_dtype, skiprows=1, delimiter=",")
        ...
        with ProgressBar(total=len(dist)) as progress:
            dist, p_order, q_order = compute_ordered_dis_njit_merge(dist, inplace=True, progress_bar=progress)
    ...
    print(f"VAT/IVAT completed. Saved column sequences to vat_col_sequence.npy and ivat_col_sequence.npy")
